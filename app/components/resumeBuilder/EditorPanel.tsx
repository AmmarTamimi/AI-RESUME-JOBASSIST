...
const GOOGLE_FONTS = [
  ...
  "Work Sans", "Source Sans Pro", "Nunito Sans", "Josefin Sans", "Quicksand",
  "Manrope", "Plus Jakarta Sans", "DM Sans", "Lexend", "Public Sans",
  "Figtree", "Epilogue", "Karla", "Playfair Display", "Merriweather",
  "Georgia", "Times New Roman", "Garamond", "Bodoni Moda", "Cormorant Garamond",
  "EB Garamond", "Libre Baskerville", "Lora", "Poppins", "Oswald", "Bebas Neue",
  "Anton", "Pacifico", "Dancing Script", "Great Vibes", "Satisfy",
  "Fira Code", "JetBrains Mono", "Source Code Pro", "Cascadia Code", "Roboto Mono",
];

const COLOR_PRESETS = [
  "#8B5CF6", "#2563EB", "#F59E0B", "#EF4444", "#10B981",
  "#EC4899", "#F97316", "#06B6D4", "#6366F1", "#14B8A6",
  "#F4A51C", "#2B2B2B",
];

const loadGoogleFont = (fontName: string) => {
  if (!fontName || ["system-ui", "sans-serif", "serif", "monospace"].includes(fontName)) return;
    ...
    switch (section.type) {
      ...
      case "skills": {
        ...
        if (typeof item === "string") {
          return (
            <div>
              <label className="block text-xs font-medium text-[#64748B] dark:text-[#94A3B8] mb-1.5">Skill</label>
              <input
                type="text"
                value={item}
      ...
      case "custom": {
        ...
        if (item && typeof item === "object" && "label" in item && "description" in item) {
          return (
            <div className="space-y-3">
              <InputField label="Label" value={item.label || ""} onChange={(v) => onUpdateItem({ ...item, label: v })} placeholder="Enter label" autoFocus />
              <InputField label="Description" value={item.description || ""} onChange={(v) => onUpdateItem({ ...item, description: v })} placeholder="Enter description" />
            </div>
          );
        }
        return null;
